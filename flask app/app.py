...
def predict_category(resume_text):
    ...
    resume_tfidf = category_vectorizer.transform([cleaned_text])
    return category_classifier.predict(resume_tfidf)[0]


# Predict top 5 job roles
def predict_top_5_roles(resume_text):
    cleaned_text = clean_resume(resume_text)
    resume_tfidf = role_vectorizer.transform([cleaned_text])
    role_probs = role_classifier.predict_proba(resume_tfidf)[0]
    top_5_indices = role_probs.argsort()[-5:][::-1]
    top_5_roles = [role_classifier.classes_[i] for i in top_5_indices]
    return top_5_roles
...
# Extract name using spaCy (first and last name)
...
# Extract education using predefined keywords
def extract_education_from_resume(text):
    ...
    education_keywords = [
        ...
        'Business Analytics', 'Operations Research', 'Decision Sciences', 'Human-Computer Interaction',
        ...
        'Web Development', 'Mobile App Development', 'Game Development', 'Virtual Reality', 'Augmented Reality',
        ...
        'Internal Medicine', 'Surgery', 'Pediatrics', 'Cardiology', 'Oncology', 'Neurology', 'Orthopedics',
        'Psychiatry',
        'Dermatology', 'Radiology', 'Anesthesiology', 'Emergency Medicine', 'Obstetrics and Gynecology', 'Pathology',
        'Urology',
        'Ophthalmology', 'Gastroenterology', 'Endocrinology', 'Pulmonology', 'Rheumatology', 'Hematology',
        'Infectious Diseases',
        'Geriatrics', 'Nephrology', 'Allergy and Immunology', 'Plastic Surgery', 'Thoracic Surgery', 'Vascular Surgery',
        'Critical Care Medicine', 'Palliative Care', 'Sports Medicine', 'Medical Education', 'Health Informatics',
        'Clinical Research',
        'Toxicology', 'Genomic Medicine', 'Precision Medicine', 'Regenerative Medicine', 'Pain Management',
        'Sleep Medicine'
    ]
...
def predict():
    ...
    skills_list = [
        ...
        'Ruby', 'Ruby on Rails', 'PHP', 'Symfony', 'Laravel', 'CakePHP', 'Zend Framework', 'ASP.NET', 'C#', 'VB.NET',
        ...
        'Patient Diagnosis', 'Treatment Planning', 'Physical Examination', 'Surgical Procedures', 'Emergency Medicine',
        'Medical Imaging Interpretation',
        'Prescription Writing', 'Patient Counseling', 'Electronic Medical Records (EMR)', 'Epic Systems', 'Cerner',
        'Meditech', 'Clinical Research',
        'Infection Control', 'Intubation', 'Central Line Placement', 'Lumbar Puncture', 'Suturing', 'Wound Care',
        'Cardiopulmonary Resuscitation (CPR)',
        'Advanced Cardiac Life Support (ACLS)', 'Pediatric Advanced Life Support (PALS)', 'Basic Life Support (BLS)',
        'Patient Education',
        'Interdisciplinary Collaboration', 'Telemedicine', 'Preventive Care', 'Chronic Disease Management',
        'Acute Care', 'Anesthesia Administration',
        'Ultrasound', 'ECG Interpretation', 'Medical Documentation', 'Quality Improvement', 'Health Policy Knowledge',
        'Time Management',
        'Empathy', 'Critical Thinking', 'Problem Solving', 'Leadership', 'Teamwork'
    ]
    skills = extract_skills_from_resume(resume_text, skills_list)
    education = extract_education_from_resume(resume_text)

    # Predict job category and roles
    job_category = predict_category(resume_text)
    top_5_roles = predict_top_5_roles(resume_text)

    # Return the extracted information along with predictions
